...
const AccountItem = ({ row }: { row: AccountRow }) => {
  ...
  const bgClass =
    row.side === 'bride' ? 'bg-rose-50/60 border-rose-100' : 'bg-sky-50/60 border-sky-100';

  return (
    <div className={`rounded-2xl border px-3 py-3 ${bgClass}`}>
      <div className="flex items-center gap-1.5 mb-2">
        <span className={`text-xs ${accentClass}`}>{row.name}</span>
        {row.isMain && (
          <HeartFillIcon className={row.side === 'bride' ? 'text-rose-400' : 'text-sky-400'} />
        )}
      </div>
      <div className="flex items-center gap-2">
        <div className="flex-1 bg-white/70 rounded-xl px-3 py-2 flex items-center justify-between border border-white/80">
          <span className="text-xs text-gray-700">{row.account}</span>
          <span className="text-[10px] text-gray-400 ml-2 shrink-0">{row.bank}</span>
        </div>
        <motion.button
          whileTap={{ scale: 0.88 }}
          onClick={handleCopy}
